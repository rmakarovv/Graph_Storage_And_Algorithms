...
public:
    ...
    virtual void removeVertex(T name) = 0;
    ...
    virtual std::vector<T> edgesFrom(T name) = 0;

    virtual std::vector<Edge<T>> edgesTo(T name) = 0;

    virtual bool hasEdge(T from, T to) = 0;
};
...
public:
    ...
    void removeVertex(T name) override {
        std::vector<std::pair<T, T>> toDelete;
        ...
        for (std::pair<T, T> &toDel : toDelete) {
    ...
    void removeEdgeBandwidth(int b) {
        ...
        for (std::pair<int, int> &el : toRemove) {
            edges.erase(el);
        }
    }
    ...
    std::vector<T> edgesFrom(T name) override {
        std::vector<T> ans;
    ...
    std::vector<Edge<T>> edgesTo(T name) override {
    ...
    void transpose() {
        std::vector< std::pair< std::pair<T, T>, Edge<T> > > edgesToReverse;
     ...
     */
    bool DFS(Vertex<T> &start, std::stack<Vertex<T>> &processed) {
        ...
        std::vector<T> adjacent = edgesFrom(start.name);
...
void general() {
    ...
    // Initializing graph.
    AdjacencyMatrixGraph<std::string> graph;
...
class Compare {
public:
    bool operator() (std::pair< std::pair<int, int>, int > p1, std::pair< std::pair<int, int>, int > p2){
...
void shortestPath() {
    ...
    for (int i = 0; i < m; i++) {
        std::cin >> source >> target >> length >> bandwidth;
    ...
    std::cin >> start >> finish >> minBandwidth;
    ...
    std::priority_queue<std::pair< std::pair<int, int>, int>,
                    std::vector< std::pair< std::pair<int, int>, int > >, Compare > queueOfVertices;
    ...
    std::map<int, std::vector<int>> shortestPaths; // shortest path between source and each vertex
    std::map<int, int> shortestLength; // Shortest length between source and each vertex
    std::vector<int> path; // For creating a shortest path.
    ...
    std::vector<bool> marked(n + 3, false);
    marked[start] = true;

    // All adjacent vertices of starting are pushed to the queue
    std::vector<int> adjacent = graph.edgesFrom(start);
    ...
    if (marked[finish]) {
        ...
        while ((i + 1) != path.size()) {
            ...
            if (bandwidthAns == -1) {
                ...
                bandwidthAns = std::min(bandwidthAns, edge.bandwidth);
            }
            i++;
        }

        // Printing number of vertices in the path, its length and bandwidth
        std::cout << path.size() << " " << lenght << " " << bandwidthAns << '\n';
